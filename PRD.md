...
31. **카드 제목 클릭 시 포커스는 원래 탭에 남게(2026-08-28)**: `target="_blank"` 기본
    동작은 새 탭을 열면서 포커스도 그리로 넘긴다 — 카드 목록을 계속 보면서 새 탭은
    백그라운드로만 열리길 원한다는 요청. `.card-title` 클릭을 가로채 `preventDefault()`
    한 뒤 `window.open(url, "_blank", "noopener")`으로 직접 열고 바로 `window.focus()`로
    원래 탭에 포커스를 돌려줌(새 탭 오픈 직후 opener를 재포커스하는 표준적인 방법).
    href는 그대로 둬서 마우스 오버 시 링크 미리보기·우클릭 메뉴(새 탭에서 열기 등)는
    그대로 살아있음. 브라우저 팝업 차단은 진짜 사용자 클릭(트러스트된 제스처)에서만
    통과되므로, 실제 마우스 클릭으로 `window.open` 호출 자체는 확인함(테스트 도구
    환경이 팝업 창 생성 자체를 막고 있어 실제 새 탭이 뜨는 것까지는 이 세션에서 직접
    확인 못 함 — 실사용 브라우저에서 재확인 필요).
