...
21. **히스토리 SNB 목록 가독성(2026-08-28)**: 날짜가 너무 작아 안 보인다는 피드백 —
...
24. **기간 검색 UI 리디자인(2026-08-28)**: "너무 올드하다"는 피드백 — 라벨+날짜input×2+
    물결+버튼×2+상태텍스트가 한 줄에 늘어서 있던 기존 UI를 **칩(chip) 하나 + 팝오버**로
    압축. 기본 상태엔 "최근 3일" 알약(pill) 버튼 하나만 보이고, 클릭하면 아래에 팝오버
    카드(날짜 범위 입력 + "최근 3일로"/"적용")가 뜸. 적용하면 칩 라벨이 `8/25 ~ 지금`
    처럼 짧게 바뀌고 파란 active 색으로 강조, 팝오버는 자동으로 닫힘. 배경 클릭·Escape로도
    닫힘(메서드 모달과 같은 패턴). 새로고침 버튼도 같은 알약 스타일 + 아이콘으로 맞춤.
    `#dateStatus`(별도 상태 텍스트)·`.date-reset`·`.date-bar-label` 등 옛 마크업/CSS는
    삭제하고 칩 라벨이 그 역할을 겸함.
25. **날짜 팝오버 안을 커스텀 캘린더 그리드로 교체(2026-08-28)**: 24번에서 칩+팝오버로
    감쌌지만 팝오버 안에는 여전히 네이티브 `<input type="date">` 2개가 있었음 — 사용자가
    다시 "달력 UI 자체가 올드하다"고 지적. 네이티브 date input을 클릭하면 뜨는 팝업
    캘린더는 브라우저/OS가 그리는 UI라 **CSS로 전혀 커스터마이징이 안 되는 것**이 근본
    이유라, input을 버리고 캘린더 그리드를 직접 그리는 방식으로 교체:
    - `app.js`의 `cal` 상태 객체(`viewYear`/`viewMonth`/`start`/`end`)+`renderCalendar()`가
      매번 42칸(6주) 그리드를 새로 그림(이전/다음 달 날짜는 `.outside`로 흐리게, 오늘은
      `.today`로 강조, 미래 날짜는 `disabled`).
    - **상호작용은 Airbnb류 range picker와 동일**: 시작일 클릭 → 종료일 클릭 순서로 범위
      선택(`onCalDayClick`), 선택 중인 범위는 `.range-start`/`.range-end`/`.in-range`
      클래스로 실시간 하이라이트. 이미 둘 다 선택된 상태에서 다시 클릭하면 새 범위로
      리셋. 시작·종료 둘 다 골라야 "적용" 버튼이 풀림(예전엔 한쪽만 입력하는 "열린 범위"도
      가능했는데, 그 기능은 범위피커 UX와 안 맞아 이번에 없앰 — 사소한 기능 축소).
    - 팝오버를 다시 열면 `state.dateRange`(마지막으로 적용한 범위)를 그대로 캘린더에
      복원해서 보여줌(`initCalFromState`).
    - `◀`/`▶` 버튼으로 월 이동. `#dateFrom`/`#dateTo` input과 관련 CSS(`.date-filter-row`)는
      완전히 삭제, `.cal-*` 클래스로 대체.
26. **기간 검색 기능 자체를 제거 — "최근 3일"만 지원(2026-08-28)**: 25번 캘린더까지 만들었지만
    사용자가 실제로 과거 날짜로 검색해보니 "선택한 기간에 해당하는 글이 없습니다"만 계속
    나옴 — RSS 아카이브(`items_store.py`, 14일 보존)는 서비스가 그 기간에 실제로 수집해둔
    글만 있어서, 서비스 시작 이전이나 트래픽이 뜸했던 날짜는 애초에 결과가 없을 수밖에
    없는 근본적 한계(9번 항목에 이미 기록된 리스크). UI를 아무리 다듬어도 이 한계는
    안 풀려서, **기간 검색 기능 자체를 걷어내고 "최근 3일"만 지원**하기로 함(사용자 결정).
    - 프런트: `state.dateRange`·`cal` 상태와 칩/팝오버/캘린더 관련 함수·이벤트 리스너를
      `app.js`에서 전부 삭제, `date-bar`는 "최근 3일" 고정 텍스트(`.recency-label`) +
      새로고침 버튼만 남김. `common.js`의 `apiGetItems()`도 `range` 인자 제거.
    - 백엔드: `/api/items`가 `days`(기본 3)만 받도록 단순화, `start`/`end`/
      `_parse_date_param`은 삭제. `_in_range(item, since)`도 `until` 없이 단방향 비교로.
    - 22~25번에서 만들었던 칩/팝오버/캘린더 UI 전체가 이번에 다시 삭제됨 — 반복된
      리디자인 끝에 "날짜 검색은 아카이브 한계 때문에 애초에 신뢰할 수 없다"는 결론에
      도달, 기능 자체를 없애는 게 UI를 계속 다듬는 것보다 나은 선택이었음.
27. **"최근 N일" 필터 추가(2026-08-28)**: 26번에서 날짜 검색을 통째로 없앤 직후, 사용자가
    "최근 3일/2일/1일" 정도의 간단한 필터는 필요하다고 요청 — 임의 과거 날짜 검색과 달리
    **이미 수집해둔 최근 범위 안에서만 고르는 거라 26번의 아카이브 한계 문제와 무관**해서
    바로 추가함. 백엔드는 이미 `/api/items?days=N`을 지원하고 있어 손 안 댐. 프런트:
    `#daysFilter`가 기존 소스 탭(`#sourceTabs`)과 같은 `.tabs`/`.tab` 스타일을 재사용해
    "1일/2일/3일" 세그먼트를 표시(기본 3일), `state.days`를 바꾸고 `loadItems()` 재호출.
    **버그 하나 잡음**: 기존 `onTabClick`이 `document.querySelectorAll(".tab")`로 활성
    상태를 전역으로 지우고 있었음 — `.tabs` 그룹이 하나뿐일 땐 문제없었지만, `#daysFilter`가
    같은 `.tab` 클래스를 쓰면서 두 그룹이 서로의 active 상태를 지우는 충돌이 생길 뻔함.
    `onTabClick`/새 `onDaysClick` 둘 다 각자의 컨테이너(`#sourceTabs`/`#daysFilter`) 안에서만
    active를 지우도록 스코프를 좁혀 해결.
28. **카드 UI 간소화(2026-08-28)**: (1) 각 카드의 "트렌드에 포함" 체크박스를 없앰 — 카드
    클릭 시 어차피 선택이 토글되게 이미 돼 있어서(제목/AI요약 버튼 클릭만 예외) 체크박스가
    중복이었음. 선택 여부는 카드 테두리 강조(`.card.selected`)로만 표시. `onSelect`가
    `state.selected` 기준으로 직접 토글하도록 바꾸고, 이제 없는 `.select-label` 관련 CSS도
    삭제. (2) "선택 항목 트렌드 분석" 버튼 문구를 **"선택 항목 인사이트 분석"**으로 변경 —
    `/analyze-insights` 슬롯 프레임 기반 분석이라는 점을 이름에 반영. (3) 헤더의
    "TrendBoard" 밑에 있던 소스 안내 문구(`.subtitle`)를 삭제, `h1` 아래 여백만 조정.
29. **"전체" 탭에서 소스가 한 덩어리로 뭉쳐 보이는 문제(2026-08-28)**: "정렬이 소스별로
    되는 것 같다"는 신고를 받고 실데이터로 확인 — `/api/items`는 이미 `published` 기준
    내림차순으로 소스 섞어 정렬하고 있었지만, **네이버 IT는 목록 페이지에 정확한
    발행시각이 없어 "수집 시각"을 대신 쓰는 게 원인**(9번 리스크 항목에 이미 기록돼있던
    한계)이었음 — 한 번 수집할 때 수백 개 기사가 똑같은 타임스탬프를 갖다 보니 정렬 시
    한 덩어리로 뭉침(실측: 상위 39개가 전부 aitimes, 그 뒤로 naver_it 353개가 몰려 나옴).
    상세 페이지까지 긁어 정확한 시각을 얻는 방법은 새로고침마다 네이버로 가는 요청이
    크게 늘어(현재 목록 1회 → 기사당 1회) 네이버 스크레이핑 취약성 리스크가 더 커지므로
    보류하고, **프런트에서 라운드로빈 대신 "전체 길이에 비례한 위치로 흩뿌리는" 방식**으로
    소스를 섞기로 함(사용자 확인). `app.js`의 `interleaveBySource()` — 소스별 항목 개수가
    크게 다를 때(naver_it 353 vs aitimes 115 vs geeknews 83) 단순 라운드로빈은 작은
    버킷이 먼저 바닥나고 큰 버킷만 뒤에 몰리는 문제가 있어서, 각 소스 항목에
    `i * (전체길이 / 그소스개수)`로 가상 위치를 매겨 그 위치 기준으로 정렬 — 각 소스
    내부의 상대적 최신순은 유지한 채 전체에 고르게 분산됨. 실측: 최대 연속 동일소스
    개수가 243개 → 4개로 감소. 진짜 "등록순"은 아니지만(네이버 IT 자체의 시각 정밀도
    한계는 그대로) 체감상 뭉침은 해소됨.
30. **"맨 위로" 버튼 추가(2026-08-28)**: "전체" 목록이 555개까지 늘어나면서 카드 페이지에
    스크롤을 많이 내려야 하는 경우가 생겨서, 우하단 고정 원형 버튼 추가(`#backToTopBtn`).
    `window.scrollY < 400`이면 숨김(`opacity`+`pointer-events`로, `display:none` 대신이라
    부드럽게 나타남), 클릭하면 `window.scrollTo({top:0, behavior:'smooth'})`.
31. **카드 제목 클릭 시 포커스를 원래 탭에 남기는 시도 — 시도했다가 되돌림(2026-08-28)**:
    `target="_blank"` 기본 동작은 새 탭을 열면서 포커스도 그리로 넘긴다 — 카드 목록을
    계속 보면서 새 탭은 백그라운드로만 열리길 원한다는 요청으로, `.card-title` 클릭을
    가로채 `preventDefault()` 후 `window.open(url, "_blank", "noopener")` +
    `window.focus()`로 포커스를 돌려주는 방식을 시도함. **실사용 확인 결과 여전히 새
    탭으로 포커스가 넘어감** — 최신 Chrome은 페이지가 `window.focus()`로 포커스를
    되찾아오는 걸 보안상 의도적으로 무시하기 때문에, 이 트릭이 더 이상 안 먹힘. 게다가
    모든 클릭을 가로채는 방식이라 브라우저 기본 제공 기능인 **가운데 클릭/Ctrl+클릭으로
    백그라운드 새 탭 열기**까지 덮어써버리는 부작용도 있었음. 웹페이지 쪽에서 강제할
    방법이 없다고 판단해 **원래의 단순 `target="_blank"` 앵커로 되돌림**(JS 핸들러
    제거). 사용자가 새 탭을 포커스 없이 열고 싶으면 가운데 클릭이나 Ctrl/Cmd+클릭을
    쓰면 됨(브라우저 네이티브 동작, 이건 그대로 살아있음).
32. **선택 항목 전체 해제 버튼 추가(2026-08-28)**: 카드 클릭으로 개별 선택/해제는 이미
    됐지만(토글), 소스 탭을 옮겨다니며 여러 개 고른 뒤에는 이전 탭에서 고른 카드가 화면에
    안 보여서 하나씩 다시 찾아가 해제해야 하는 문제 — 트렌드바에 **"선택 해제"** 버튼을
    추가(`#clearSelectionBtn`, 1개 이상 선택됐을 때만 노출). `onClearSelection()`이
    `state.selected`(현재 탭 필터와 무관하게 전역 상태)를 통째로 비우고, 화면에 남아있는
    카드가 있으면 `.selected` 클래스도 같이 지움 — 다른 탭으로 이동해 안 보이는 카드까지
    한 번에 해제됨(확인함: GeekNews 탭에서 해제 → 전체 탭으로 돌아가도 선택 0개).
    레이아웃상 트렌드바를 "탭 그룹 / (해제+분석 버튼)" 2단으로 유지하려고 `trendBtn`을
    `.trend-actions` div로 한 번 더 감쌈.
